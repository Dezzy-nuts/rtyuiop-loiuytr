...
def mainWin():
    ...
    imglocation = "data/images/spandark.jpg"
    mainimg = Image.open("data/images/LUNA.gif")
    mainFrameContainer = []
    try:
        while True:
            frame = mainimg.copy().resize((876, 715), Image.LANCZOS)
    ...
    btnfont = ("Verdana", 15, "bold")  # font used in widgets
    ...
    title_program = tk.Label(main, text='PC PERFORMANCE MANAGER', bg='#000000', font='Verdana 30 bold', fg='#9a87ad')
    
    cpu_title_label = tk.Label(main, text='CPU Usage: ', bg='#000000', font='Verdana 24 bold', fg='#ffffff')

    ram_title_label = tk.Label(main, text='RAM Usage: ', bg='#000000', font='Verdana 24 bold', fg='#ffffff')

    cpu_Label = tk.Label(main, bg='#000000', fg='#FA5125', font='Verdana 30 bold', width=20)

    ram_Label = tk.Label(main, bg='#000000', fg='#FA5125', font='Verdana 30 bold', width=20)
    ...
    AeolianVoice = tk.Button(sidebar, text="Luna Voice",fg=color["t"], font=btnfont, bg=color["p"], borderwidth=0, padx=50, cursor="hand2",command=lambda: aeolian(main, color), highlightthickness=0)
...
def viewSettings(main, color, config, default):
    ...
    resize = img.resize((125,50), Image.LANCZOS)
    ...
    resize2 = img2.resize((125,50), Image.LANCZOS)
...
def howToUse(main, color):
    ...
    try:
        while True:
            frame = img1.copy().resize((256, 256), Image.LANCZOS)
            frames1.append(ImageTk.PhotoImage(frame))
            img1.seek(len(frames1))
    except EOFError:
        pass
    # resized = img2.resize((256, 256), Image.LANCZOS)
    ...
    try:
        while True:
            frame = img2.copy().resize((256, 256), Image.LANCZOS)
...
def aeolian(main, color):
    ...
    voiceBtn = tk.Button(aeolianContent, text="Luna Voice Assistant", font=('Arial', 18), bg=color["s"], fg=color["t"], relief="groove", borderwidth=2, highlightthickness=1, cursor="hand2",command = lambda:ask1(AolianLabel) )
